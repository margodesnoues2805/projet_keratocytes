...
"""
 la fonction permet de sauvegarder nos resultats dans un dataframe.
    ...
    Les arguments sont :
            ...
            aire_hist : historique des aires à chaque pas d'enregistrement* (list)
            filename : Nom du fichier pour le télechargement des données et de la vidéo
 
    La fonction sauvegarde les listes dans un dataframe .csv via la librairie panda.
...
def Sauvegarde_simulation_csv(x_hist, y_hist, etat_hist, bary_hist, aire_hist, filename="simulation"):
    ...
    aire_arr = np.array(aire_hist)   
    
    # 
    n_steps, N = x_arr.shape
    ...
    df.to_csv(filename + "_datafile.csv", index=False)
